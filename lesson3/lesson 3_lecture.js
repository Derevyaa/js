...
// Наприклад, коротший спосіб написання while (iw != 0) відповідає while (i):
let iw = 10;
while (iw) { // коли i буде 0, умова стане невірною, і цикл зупиниться
    console.log(iw);
    iw--;
}
...
//погане рішення
/**
 *  є логічна помилка, оскільки він має виводити тільки парні числа, 
 * але через умову if (x % 2) виводяться непарні числа. Умова x % 2 повертає true для непарних чисел, тому що результат ділення на 2 має залишок 1.
 */
for (let x = 0; x < 10; x++) {
...
//давайте пофіксимо
/**
 * Цей цикл коректно пропускає вивід непарних чисел, використовуючи continue для переходу до наступної ітерації, коли умова (x % 2) виконується (тобто для непарних чисел).
 */
for (let x = 0; x < 10; x++) {
    if (x % 2) continue;  // пропустити непарні числа
...
/**
 * Другий цикл з break
Цей цикл припинить своє виконання при першому непарному числі, що може бути неочікуваною поведінкою, якщо ви хочете вивести всі парні числа до 10.
 */
for (let x = 0; x < 10; x++) {
    if (x % 2) break;  // вихід з циклу при першому непарному числі
    console.log('тільки парні: х = ' + x);
}
console.log("Цикл було передчасно перервано.");
...
Мітки для циклів у JavaScript можуть бути корисними, коли ви працюєте з багаторівневими вкладеними циклами 
і потребуєте керувати переходами або виходом із них на певному рівні. Ось декілька прикладів використання міток з циклами for та while в JavaScript.

Приклад 1: Використання мітки з break
Уявіть, що у вас є вкладені цикли, де вам потрібно припинити виконання усіх циклів, коли певна умова є правдивою. Без мітки, break припинить тільки найближчий цикл.

Використання мітки з break

Мітка: Це ідентифікатор, який вказується перед циклом і може використовуватися з операторами break або continue для управління виконанням циклу на різних рівнях вкладеності.

break: Оператор, що використовується для негайного припинення циклу. Без мітки, break завершує найближчий охоплюючий цикл. 
Якщо використовувати break з міткою, він припиняє виконання циклу, до якого прив'язана мітка.

Приклад пояснення:
У прикладі з міткою outerLoop і двома вкладеними циклами for, коли зустрічається умова (i === 2 && j === 2), 
оператор break outerLoop; припиняє весь зовнішній цикл, а не тільки внутрішній, де знаходиться умова.
*/
...
outerLoop: for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 5; j++) {
        console.log(`i=${i}, j=${j}`);
        if (i === 2 && j === 2) {
            break outerLoop; // Припиняє весь зовнішній цикл
        }
    }
}

let exit = false;  // Зовнішня змінна для контролю виходу

for (let i = 0; i < 5 && !exit; i++) {
    for (let j = 0; j < 5; j++) {
        console.log(`i=${i}, j=${j}`);
        if (i === 2 && j === 2) {
            exit = true;  // Встановлюємо змінну для виходу
...
/**Приклад 2: Використання мітки з continue
Мітки можуть бути також використані з оператором continue, щоб продовжити виконання на вищому рівні циклу замість найближчого.

Використання мітки з continue

continue: Оператор, що використовується для пропуску поточної ітерації циклу і продовження наступної ітерації. 
Зазвичай, continue впливає лише на цикл, в якому він викликаний. Використання continue з міткою дозволяє продовжити на наступну ітерацію циклу, асоційованого з цією міткою.

Приклад пояснення:
У прикладі з міткою outerLoop оператор continue використовується внутрішнім циклом. Коли зустрічається умова (j % 2 === 0), 
оператор continue; спричиняє пропуск поточної ітерації внутрішнього циклу і продовження з наступною ітерацією цього ж циклу.
*/
outerLoop: for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 5; j++) {
        if (j % 2 === 0) {
            continue; // Пропускає лише поточний прохід внутрішнього циклу
        }
        console.log(`i=${i}, j=${j}`); // Виконується тільки для непарних j
    }
}

for (let i = 0; i < 5; i++) {
    let skipIteration = false; // Змінна для управління пропуском ітерацій

    for (let j = 0; j < 5; j++) {
        if (j % 2 === 0) {
            skipIteration = true; // Вказуємо пропускати ітерацію
        ...
        skipIteration = false; // Скидуємо змінну для наступної ітерації
    }
}


/**
 * Приклад 3: Комбінація мітки, break, і continue
Можна комбінувати break і continue з мітками для складніших сценаріїв керування вкладеними циклами.

Комбінація мітки, break, і continue

Приклад пояснення:
Тут ми маємо складнішу структуру з трьома вкладеними циклами і двома мітками: outerLoop і innerLoop. Коли виконується умова (k === 1), 
continue innerLoop; веде до пропуску решти виконання циклу, асоційованого з міткою innerLoop, і переходить до наступної ітерації цього циклу. 
Коли зустрічається умова (j === 2 && k === 2), break outerLoop; негайно припиняє виконання усіх циклів, асоційованих з outerLoop.
 */
outerLoop: for (let i = 0; i < 3; i++) {
    innerLoop: for (let j = 0; j < 3; j++) {
        for (let k = 0; k < 3; k++) {
            if (k === 1) continue innerLoop; // Пропускає решту циклу `innerLoop`
            console.log(`i=${i}, j=${j}, k=${k}`);
            if (j === 2 && k === 2) {
                break outerLoop; // Виходить з усіх циклів до мітки `outerLoop`
            }
        }
    }
}
...
for (let i = 0; i < 3 && !exitAll; i++) {
    for (let j = 0; j < 3 && !exitAll; j++) {
        for (let k = 0; k < 3; k++) {
            if (k === 1) {
                break;  // Пропускаємо решту ітерацій внутрішнього циклу
            }
            console.log(`i=${i}, j=${j}, k=${k}`);

            if (j === 2 && k === 2) {
                exitAll = true;  // Змінна для виходу з усіх циклів
                break;
            }
        }
    }
...
function squareAreaCalculation1(side) {
...
let result = squareAreaCalculation(squareAreaCalculation1(5));
